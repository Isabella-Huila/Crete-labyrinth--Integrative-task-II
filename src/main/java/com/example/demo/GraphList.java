...
public class GraphList<K> implements IGraph<K> {
    ...
    public ArrayList<Edge<K>> kruskal(ArrayList<Edge<Integer>> edges) {
        ArrayList<Edge<K>> minimumSpanningTree = new ArrayList<>();
        Collections.sort(edges, Comparator.comparingInt(Edge::getCost)); // Ordena las aristas de forma creciente
        for (Edge<Integer> edge : edges) {
            Vertex<K> origin = (Vertex<K>) edge.getOrigin();
            Vertex<K> destination = (Vertex<K>) edge.getDestination();

            if (!hasCycle(minimumSpanningTree, origin, destination)) {
                minimumSpanningTree.add((Edge<K>) edge);
            }
        }
